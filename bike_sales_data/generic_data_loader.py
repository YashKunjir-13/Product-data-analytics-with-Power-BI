    ...
    def _detect_column_types(self):
        ...
        for col in self.df.columns:
            ...
            if 'date' in col.lower() or 'time' in col.lower():
                ...
                # Sample a few values to see if they are dates
            ...
            if is_numeric:
                # Filter out ID-like columns from numeric metrics
                if not any(id_keyword in col.lower() for id_keyword in ['id', 'uuid', 'guid', 'pk', 'fk']):
                    self.numeric_columns.append(col)
                    self.column_types[col] = 'numeric'
                else:
                    self.column_types[col] = 'identifier'
                    # We still treat it as numeric if it is, but it's not a "metric"
                    # For now, let's keep it in numeric_columns if it's actually numeric, 
                    # but maybe we should have a separate list for metrics vs all numeric.
                    # The current engine uses numeric_columns for metrics.
                    pass 
            else:
